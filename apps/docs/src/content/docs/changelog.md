...
- **DeviceSDK is now free, open-source, and self-hosted.** The managed cloud has been replaced by a server you run yourself. Everything - REST API, device & watcher WebSockets, and the dashboard - is now a single **Bun** process on one port (default `8080`), distributed as a multi-arch **Docker image** (`ghcr.io/device-sdk/devicesdk`) that runs on a Raspberry Pi, NUC, NAS, or any Docker host. All state (SQLite database, device scripts, firmware) lives under a `/data` volume you control.
  - **Open source under AGPL-3.0.** See the [LICENSE](https://github.com/device-sdk/devicesdk/blob/main/LICENSE) in the repository.
...
- **`@devicesdk/mcp` (new)** - Model Context Protocol stdio server that exposes 7 DeviceSDK tools to AI coding agents (Claude Desktop, Claude Code, Cursor, Continue.dev, Windsurf). See [`/mcp/`](/mcp/) for install snippets per host.
- **AI-agent friendliness pass:**
  - `devicesdk init` now scaffolds `AGENTS.md`, `.cursor/rules/devicesdk.mdc`, `.mcp.json`, and a project `README.md`.
  ...
  - Auth errors now carry stable `code` and `docs` fields. See the new [error reference](/errors/).
  ...
  - New cookbook at [`/recipes/`](/recipes/) with 10 task-shaped recipes.
  - URL change: `/resources/changelog/` is now `/changelog/` (the old URL 301s).
- **Removed:** the deprecated SSE log stream endpoint (`GET /logs/stream`). Use the [watch WebSocket](/guides/real-time-watch/) instead - the dashboard, CLI, and Home Assistant integration already do.

## April 11, 2026

- **Home Assistant integration** - expose DeviceSDK devices as native Home Assistant entities (sensors, switches, lights). Declare entities in `devicesdk.ts` under `ha.entities`; run `devicesdk deploy` to publish them. See the [Home Assistant guide](/guides/home-assistant/).
- **Generic watch WebSocket** - new `GET /v1/projects/:projectId/devices/:deviceId/watch` endpoint delivers real-time status, log, and structured state events over a persistent WebSocket connection. The dashboard now uses this endpoint in place of the legacy SSE log stream. See the [Real-Time Watch guide](/guides/real-time-watch/).
- **`emitState` SDK method** - publish structured state values from device scripts with `this.env.DEVICE.emitState(entity_id, value)`. Feeds custom telemetry into Home Assistant entities. See the [Emit State concept](/concepts/emit-state/).
