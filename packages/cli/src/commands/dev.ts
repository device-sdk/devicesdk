...
import { DeviceSDKConfig, DeviceConfig } from '../config.js';
...
const generateCapnpConfig = (devices: Record<string, DeviceWithClass>, entrypointPath: string, simulatorAssetsPath: string, port: number): string => {
    const durableObjects = Object.keys(devices).map(key => `
      (className = "${devices[key].className}", enableSql = true)
    `).join(',');

    const bindings = Object.entries(devices).map(([key, value]) => `
...
const generateWorkerdEntrypoint = async (devices: Record<string, DeviceWithClass>, tmpDir: string): Promise<string> => {
    const entrypointPath = path.join(tmpDir, '_workerd_entry.ts');
    const imports = Object.values(devices).map(device => `import { ${device.className} as ${device.className} } from '${device.resolvedEntrypoint}';`).join('\n');
    const exports = `export { ${Object.values(devices).map(device => device.className).join(', ')} };`;
    const content = `${imports}\n${exports}`;
    await fs.writeFile(entrypointPath, content);
    return entrypointPath;
};

const buildEntryPoint = async (entrypointPath: string, outfile: string, extraLoaders: esbuild.Plugin[] = []) => {
    await esbuild.build({
...
const dev = async (options: { config?: string }) => {
    ...
    try {
        ...
        console.error(`Error: Could not find ${configPath}. Make sure the file or directory exists.`);
        process.exit(1);
    }
    ...
    try {
        ...
        const entrypointPath = await generateWorkerdEntrypoint(devicesWithClass, tmpDir);
        const outfile = path.join(tmpDir, 'bundle.js');
        await buildEntryPoint(entrypointPath, outfile);

        const simulatorEntrypointPath = path.resolve(__dirname, '../simulator/worker.ts');
        ...
        await buildEntryPoint(simulatorEntrypointPath, simulatorOutfile, [
            {
                ...
                setup(build) {
                    build.onResolve({ filter: /\?raw$/ }, (args) => ({
                        path: path.isAbsolute(args.path) ? args.path.slice(0, -4) : path.join(args.resolveDir, args.path.slice(0, -4)),
        ...
        const capnpConfig = generateCapnpConfig(devicesWithClass, 'bundle.js', path.resolve(__dirname, '../simulator/assets'), port);
...
`);
        ...
        await execa('workerd', ['serve', 'config.capnp', '--verbose'], { stdio: 'inherit', cwd: tmpDir });
