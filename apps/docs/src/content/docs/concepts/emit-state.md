...
`this.env.DEVICE.emitState(entityId, value)` publishes a structured state update from a device script. Every subscriber on the device's [watch WebSocket](/guides/real-time-watch/) receives the value as a `state` event. Home Assistant uses these events to update the matching entity in real time.
