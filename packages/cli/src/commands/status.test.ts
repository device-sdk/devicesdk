...
describe("status command", () => {
	...
	it("exits 3 when project has no devices", async () => {
	...
	it("exits 3 when --device flag specifies unknown device", async () => {
		apiMocks.listDevices.mockResolvedValue([makeDevice("sensor-1")]);

		await expect(status({ device: "unknown" })).rejects.toThrowError(/exit:3/);
		expect(exitSpy).toHaveBeenCalledWith(3);
	});
