...
async function listVolumes(): Promise<Array<{ mountpoint: string; label?: string }>> {
  ...
  if (platform === 'darwin') {
    ...
    for (const line of lines) {
      ...
      } else if (trimmedKey === 'Partition Type' && current.mountpoint && current.label !== undefined) {
        volumes.push({ mountpoint: current.mountpoint, label: current.label });
        current = {};
      }
    }
    return volumes.filter((v) => v.mountpoint && v.mountpoint !== 'Not mounted');
  }

  if (platform === 'linux') {
    const { stdout } = await execa('lsblk', ['-o', 'NAME,LABEL,MOUNTPOINT', '-P', '-p']);
    return stdout
      .split('\n')
      .map((line) => {
      ...
      .filter((v): v is { label?: string; mountpoint: string } => Boolean(v.mountpoint));
  }

  throw new Error(`Unsupported platform: ${platform}`);
}

async function findPicoMount(volumeLabel: string | string[], timeoutMs: number): Promise<string> {
  const labels = Array.isArray(volumeLabel) ? volumeLabel : [volumeLabel];
  const start = Date.now();
  while (Date.now() - start < timeoutMs) {
    const volumes = await listVolumes();
    const match = volumes.find(
      (v) => labels.some((label) => (v.label || '').trim() === label) && v.mountpoint
    );
    if (match?.mountpoint) {
  ...
  throw new Error(`Timed out waiting for Pico volume "${labels.join('" or "')}"`);
}

export async function flashPico(options: PicoFlashOptions): Promise<{ mountpoint: string }> {
