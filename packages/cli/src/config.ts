...
const deviceTypeSchema: z.ZodType<DeviceType> = z.enum(['pico-w', 'pico2-w', 'esp32']);
...
export const DeviceConfigSchema = z.object({
  ...
  } as { main: string; entrypoint: string; deviceType: DeviceType; name?: string; description?: string; wifi: { ssid: string; password: string } };
});

export const DeviceSDKConfigSchema = z.object({
  projectId: z.string().min(1).max(36).regex(/^[a-z][a-z0-9-]{0,35}$/),
  devices: z.record(z.string(), DeviceConfigSchema),
});

export type DeviceSDKConfig = z.infer<typeof DeviceSDKConfigSchema>;
export type DeviceConfig = Omit<z.infer<typeof DeviceConfigSchema>, 'deviceType'> & { deviceType: DeviceType };
