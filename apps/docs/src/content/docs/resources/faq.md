...
See the [Hardware Compatibility](/hardware/) pages for per-board details.
...
The CLI generates `devicesdk-env.d.ts` with types for all devices in your project. Methods that only use KV storage work even when the target device's hardware is offline. See the [Inter-Device Communication Guide](/guides/inter-device-communication/) for a full walkthrough.
...
- Check our [troubleshooting guide](/resources/troubleshooting/)
