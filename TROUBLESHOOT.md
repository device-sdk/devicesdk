...
### `pnpm/action-setup@v5`/`@v6` fails on the self-hosted runner (`self-installer exits with code 1`)
**Date**: 2026-05-29
**Question/Problem**: Bumping `pnpm/action-setup` from `@v4` to `@v6` (as part of moving every GitHub Action onto its Node 24-native major) made *every* CI job fail at the "Run pnpm/action-setup" step within ~15 s, before any test logic ran. The log shows `npm warn cli npm v11.12.1 does not support Node.js v18.19.1`, then `TypeError [ERR_INVALID_ARG_TYPE]: The "paths[0]" argument must be of type string` inside `node_modules/@pnpm/exe`'s `node setup.js`, ending with `Something went wrong, self-installer exits with code 1`.
**Root Cause**: In our workflows `pnpm/action-setup` runs **before** `actions/setup-node` (it has to — `setup-node`'s `cache: 'pnpm'` needs `pnpm` already on PATH). So when `pnpm/action-setup` runs, the only Node available is the runner image's **system Node, v18.19.1** (EOL). `@v5`/`@v6` install pnpm via `npm install @pnpm/exe`, and the system `npm` is v11, which only supports Node `^20.17.0 || >=22.9.0` — so the install crashes on Node 18. `@v4`'s installer tolerates Node 18, which is why CI was green before the bump. The action's own `runs.using: node24` does **not** help: the failure is in the `npm install` subprocess, which inherits PATH's system Node, not the action runtime.
**Solution**: Pin `pnpm/action-setup` to `@v4` while every other action stays on its Node 24 major. The proper long-term fix is to bump the **self-hosted Proxmox runner image's system Node to ≥20** (ideally 24, to match `.nvmrc`); once that's done, `pnpm/action-setup` can move to `@v6`.
**Rule**: Do **not** bump `pnpm/action-setup` past `@v4` until the runner image's system Node is ≥20. It is deliberately the one action left on a non-latest major (see the commit "ci: keep pnpm/action-setup at v4"). This is a runner-image limitation, not a workflow bug — per the CI Runner Image policy in CLAUDE.md, raise the system-Node bump with the image maintainer rather than reordering jobs or hacking around it.
