...
export class GetDeviceMetrics extends BaseRoute {
	public schema = {
		...
		responses: {
			"200": {
				description: "Estimated usage metrics over the requested window",
				...contentJson(
					z.object({
						success: z.boolean(),
						result: z.object({
	...
	public async handle(c: AppContext) {
		...
		// Usage is recorded under the device's UUID (see device.ts), not its slug.
		const buckets = await fetchDeviceSeries(c.env, device.id, window);
