...
import { startAuth, pollAuth, getMe, setVerbose } from '../api.js';
import { saveCredentials, Credentials } from '../credentials.js';
...
export default async function login(options?: { verbose?: boolean }): Promise<void> {
  if (options?.verbose) {
    setVerbose(true);
    isVerbose = true;
  }
