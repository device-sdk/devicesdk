...
| File | Responsibility |
|------|----------------|
| `main/iotkit_main.c` | App entry: Wi-Fi bring-up, WebSocket task, event loop, response building, boot/OLED status. |
...
| `main/hal.c/.h` | Hardware abstraction layer (`iotkit_hal_*`) — GPIO/PWM/ADC/I2C/SPI/UART/temperature/watchdog/LED/reboot. |
...
| `main/config.h`, `main/Kconfig.projbuild` | Compile-time config placeholders + the *IoTKit Configuration* menuconfig (onboard LED pin/type). |

## Boot & connection flow

1. **Boot** — `display_boot_text("Booting")`, `iotkit_hal_blink_led(1)`, init NVS and the HAL.
2. **Wi-Fi** — connect as a WPA2 station to the configured SSID/password; on success
   `display_boot_text("WiFi")`, `iotkit_hal_blink_led(2)`.
3. **WebSocket** — build the endpoint and connect via `esp_websocket_client`:
   ...
   - `display_boot_text("Server")` while connecting; on `WEBSOCKET_EVENT_CONNECTED`,
     `display_boot_text("Connected")` + `iotkit_hal_blink_led(3)`, and the device sends a
     `{"type":"device connect"}` frame.
4. **Keepalive** — a periodic ping (`IOTKIT_PING_INTERVAL_MS`, 5 minutes) keeps the
   connection warm.
...
- The **WebSocket task** (`iotkit_main.c` event handler → `websocket_handler.c`) parses each
...
**Device → server responses/events** (built in `iotkit_main.c`):
...
All hardware access goes through `main/hal.h` (`iotkit_hal_*`), so the command handlers stay
