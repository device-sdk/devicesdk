...
**Root Cause**: The Dockerfile fetched the newest firmware GitHub release at image build time (a `curl` against the releases API in a `firmware` stage), racing the ~60-minute ESP32 firmware workflow that publishes the new release. The build-time curl completed first, so the image picked up the not-yet-published version's predecessor. The race was inherent: image builds and firmware publishing run on independent schedules.
**Solution**: The server now depends on `@devicesdk/firmware-esp32` / `@devicesdk/firmware-pico` (`workspace:*`), so changesets bump the server in lockstep with every firmware release, and `apps/server/src/foundation/firmwareSync.ts` fetches exactly the pinned release version at boot. No firmware is baked into the image anymore (the Dockerfile has no firmware stage), so a freshly built image always serves the firmware it was released with. `docker.yml` additionally waits for the pinned firmware releases to exist before publishing the image and **fails on timeout**, so a published image never 404s on first boot.
**Rule**: If a pinned firmware release is missing (e.g. the firmware build failed), `syncFirmware` logs `sync failed` and the flash endpoint reports "not published" until the binaries exist, and the docker.yml `wait-for-firmware` job fails the build with an instruction to re-run the firmware workflow. Note that `syncFirmware` never fetches a NEWER version than the pinned one - that is intentional (reproducible server+firmware pairs). Marker files `.version-esp32` / `.version-pico` in `DATA_DIR/firmwares/` make boot idempotent; a partial download leaves the previous binaries untouched.

### CLI `tsc` fails in a fresh worktree with "Type '"onewire_search"' is not assignable to type ..." even though `@devicesdk/core` source has the type
**Date**: 2026-08-12
**Question/Problem**: In a newly added worktree (no `node_modules`), running `pnpm test --filter @devicesdk/cli` (or `tsc`) fails with dozens of `TS2322` errors in `packages/cli/src/simulator/localDeviceSender.ts` - `Type '"onewire_search"' is not assignable to type '"get_pin_state" | ...'` and similar for `dht_read`. `packages/core/src/commands.ts` clearly defines `OnewireSearchCommand` / `DhtReadCommand` and includes them in `DeviceCommand`. It looks like a pre-existing type bug on `main` (the simulator was added in #227).
**Root Cause**: It is not a code bug. The CLI resolves `@devicesdk/core` to the workspace package via a `node_modules/@devicesdk/core` symlink, and `packages/core/package.json`'s `exports` map points its `types` at `./dist/index.d.ts`. A fresh worktree has no `packages/core/dist` (it is gitignored and only produced by the build), so tsc resolves `@devicesdk/core` to nothing useful and reports the *missing* onewire/dht types as if the union lacked them. CI doesn't hit this because `turbo run test` / `check-types` builds `@devicesdk/core` first via topological ordering; only a bare `tsc` in a fresh worktree does.
**Solution**: Run `pnpm install`, then `pnpm build --filter @devicesdk/core` before `tsc`/CLI tests. Never interpret these errors as a onewire/dht typing regression; check `ls packages/core/dist` first.
**Rule**: After `git worktree add` on this repo, `@devicesdk/core` has no `dist` until you build it - the CLI and server `tsc` gates fail with misleading "not assignable to type" errors (not `Cannot find module`) until then. Build core (or run the full `pnpm build`) before touching tsc-based gates in a fresh worktree.

### `expectTypeOf(value)` with a string-literal or union type fails with TS2344 "does not satisfy the constraint 'Expected: ..., Actual: ...'"
**Date**: 2026-08-12
**Question/Problem**: In `packages/core/tests/types/deviceType.test-d.ts`, value-based assertions like `expectTypeOf("pico-w").toMatchTypeOf<DeviceType>()` and `expectTypeOf(x).toEqualTypeOf<DeviceType>()` (where `x` is a declared `DeviceType` union) fail at typecheck with `TS2344: Type 'DeviceType' does not satisfy the constraint '"Expected: literal string: pico-w, Actual: never" | ...'`. The same assertions with **explicit type arguments** (`expectTypeOf<"pico-w">().toMatchTypeOf<DeviceType>()`) compile fine.
**Root Cause**: expect-type 1.3.0 builds the `Expected` type parameter's constraint as a distributive conditional over the actual type (`StrictEqualUsingBranding<Actual, Expected> extends true ? unknown : MismatchInfo<Actual, Expected>`). When the actual is a multi-member union (or a literal inferred from a value), the constraint distributes into a union of per-member `MismatchInfo` error-string types, which the union `Expected` argument can never satisfy - so the constraint check itself errors even though the assertion would be true. Value-based literal inference hits a related widening quirk (`Actual: string`).
**Solution**: In type tests, always pass explicit type arguments for literal/union assertions: `expectTypeOf<"esp32c3">().toMatchTypeOf<DeviceType>()` and `expectTypeOf<DeviceType>().toEqualTypeOf<(typeof DEVICE_TYPES)[number]>()`. Object-shaped actuals (the existing `commands.test-d.ts` pattern) are unaffected because their branded-equality path doesn't distribute the same way.
**Rule**: With expect-type 1.3.0 (repo's pinned version), never assert on string-literal unions via `expectTypeOf(value)` or `expectTypeOf(unionTypeVar)` - use explicit type arguments on both sides. If a `test-d.ts` line errors with a constraint made of `"Expected: literal string: X, Actual: never"` strings, that's this distribution bug, not a real type mismatch.

### `workerd serve` fails to parse a generated config.capnp: any `//` comment breaks the parse with a misleading error
**Date**: 2026-08-08
**Question/Problem**: `devicesdk dev` (packages/cli, workerd simulator) died at startup with `config.capnp:18:74: Parse error. The config file does not define any top-level constants of type 'Config'.` Line 18 was a `//` comment in the generated config. Reproduced minimally: a clean config containing only `  // hello world` fails (`config.capnp:13:6: Parse error`), while the identical config with `  # hello world` parses and runs. The reported line/column is unreliable (it pointed at the end of a comment line and, in one variant, at an apostrophe inside the comment), and the follow-up "does not define any top-level constants of type 'Config'" is what workerd prints for any parse error it cannot attribute to a constant - a red herring.
**Root Cause**: Cap'n Proto's schema language uses `#` for line comments, **not** `//` (that is a protobuf/C-style convention). `//` was never valid capnp; this workerd (2026-02-05) rejects it with a parse error whose position does not point at the real cause. The earlier theory that an apostrophe in the comment terminated it early is wrong - apostrophe-free `//` comments fail identically.
**Solution**: Use `#` comments in generated capnp template strings in `commands/dev.ts` (and any hand-written `config.capnp`). When workerd reports a parse error against a generated config, first grep the generated file for `//` before debugging anything else.
**Rule**: Cap'n Proto comments start with `#`. Never emit `//` comments into `.capnp` files (generated or hand-written) - workerd reports them as cryptic parse errors. A "no top-level constants of type 'Config'" error message means "some parse error earlier in the file"; check for `//` first.

### `FsBlobStore.list` cursor silently skips keys when paginating callers delete between pages (delete project/device/purge leave orphan blobs)
**Date**: 2026-08-08
**Question/Problem**: `deleteProject`/`deleteDevice`/`purgeUserData` drain blob stores with a `do { list({prefix, cursor}) } while (truncated)` loop. With >1000 blobs (the default page size), the loop stopped after the first page: e.g. 1100 seeded blobs left exactly 100 orphans after project deletion. Reproduced standalone with a 1100-key store: page 1 returned 1000 objects (truncated=true), page 2 returned 0 objects even though 100 remained.
**Root Cause**: `FsBlobStore.list` implemented the cursor as a **numeric offset** (`Number.parseInt(cursor)`, `keys.slice(offset, offset+limit)`) re-applied to a fresh `walk()` on every call. R2's real contract is a stable continuation token: already-returned keys may vanish between pages. Once the delete loop removed page 1's 1000 keys, the offset pointed past the end of the shrunken key list, so the tail page came back empty and the loop terminated early. `purgeUserData`'s loop had the same latent bug - it was the "reference pattern" for the other two.
**Solution**: Changed the cursor to the **last returned key** (R2-style continuation token): filter `k > after` instead of `keys.slice(offset, offset + limit)`, `truncated = keys.length > limit`, cursor = last page key. Unit test added in `tests/unit/fs-blob-store.test.ts` (delete-between-pages survives) and an e2e test seeding 1100 blobs through the real delete endpoint.
**Rule**: The fs blob store's cursor must be a position in key-space (last-key token), never an offset into a live walk - offset cursors break the moment a paginating caller mutates the collection between pages. Also: verify pagination with more than one page of data, or a `truncated: true` assertion alone will not catch a loop that silently stops after page 1.

### `release.yml` publish step fails with npm `422 Unsupported GitHub Actions runner environment: "self-hosted"`
**Date**: 2026-07-30
**Question/Problem**: The `Release` workflow's `Create Release PR or Publish` step (`changesets/action` running `pnpm release`) failed publishing both `@devicesdk/cli` and `@devicesdk/core` with `E422 422 Unprocessable Entity - PUT https://registry.npmjs.org/... - Error verifying sigstore provenance bundle: Unsupported GitHub Actions runner environment: "self-hosted". Only "github-hosted" runners are supported when publishing with provenance.` Auth itself succeeded (npm logged `Publishing "@devicesdk/core" at "1.4.6"` and even wrote a provenance statement to the sigstore transparency log) - only the registry-side check rejected it.
**Root Cause**: `release.yml` has `permissions: id-token: write` and no `NPM_TOKEN` secret, so npm CLI >=11.5.1 automatically uses OIDC trusted publishing, which auto-enables provenance. npm's registry hard-requires the publishing job to run on a **github-hosted** Actions runner for provenance/trusted-publishing verification - this is enforced server-side regardless of how well the OIDC token itself validates. The `release` job had been moved to `[self-hosted, linux, proxmox-ephemeral]` in the runner-migration PR (#79), which silently broke every publish since (both packages 422 every time; no partial-publish state to clean up).
**Solution**: Keep OIDC trusted publishing (don't add `NPM_TOKEN`), but move only the `release` job's `runs-on` back to `ubuntu-latest`. The repo is public, so github-hosted minutes are free; all other CI jobs stay on the self-hosted fleet.
**Rule**: If a workflow job publishes to npm with OIDC trusted publishing / provenance, it must run on a github-hosted runner - self-hosted runners cannot satisfy this no matter how the OIDC/id-token permissions are configured. Don't diagnose this as a token/auth problem; the registry response naming `"self-hosted"` explicitly is the tell.

### `docker.yml` build fails at the docs-index step with "Docs directory not found", even after confirming `.dockerignore` doesn't exclude `apps/docs`
**Date**: 2026-07-30
**Question/Problem**: The Docker image build failed on `RUN cd apps/server && bun run scripts/build-docs-index.ts apps/docs/src/content/docs /out/docs-index.sqlite` with `Docs directory not found: /repo/apps/server/apps/docs/src/content/docs`. This looks like the earlier `.dockerignore`-exclusion bug (see the entry below), but `.dockerignore` only excludes `apps/website`, not `apps/docs` - the files are in the build context.
**Root Cause**: `build-docs-index.ts` resolves its first CLI arg with `resolve(process.cwd(), argv[2])`. The Dockerfile `RUN` does `cd apps/server` first, so `process.cwd()` at run time is `/repo/apps/server` - but the arg passed was `apps/docs/src/content/docs`, a path meant to be relative to the **repo root** (`/repo`), not to `apps/server`. Resolved against the wrong cwd it becomes `/repo/apps/server/apps/docs/src/content/docs`, which doesn't exist. Introduced in #222 when the docs path/content moved and this RUN step's argument wasn't updated to match the `cd`.
**Solution**: Changed the arg to `../../apps/docs/src/content/docs` (relative to `apps/server`, matching the adjacent `bun build src/server.ts` RUN's `cd` pattern) instead of dropping the `cd`.
**Rule**: When a Dockerfile `RUN` does `cd <subdir> && <command with a path arg>`, any relative path arg is resolved against `<subdir>`, not the stage's `WORKDIR`/repo root - check both the shell's cwd *and* how the invoked script resolves its own argv relative paths (`resolve(process.cwd(), ...)` here) before assuming a `.dockerignore` problem.

### `docker build` fails on a docs-consuming build step with "directory not found", even though the Dockerfile does `COPY . .`
**Date**: 2026-07-08
**Question/Problem**: Adding a `RUN` step to the `serverbuild` stage that reads docs Markdown (`apps/server/scripts/build-docs-index.ts`, for the `/mcp` docs-search FTS5 index) failed with `Docs directory not found`, even though stage 1 does `COPY . .` into `/repo` and stage 2 does `COPY --from=build /repo /repo`.
**Root Cause**: `.dockerignore` had a bare `docs` line. `.dockerignore` exclusions apply to the **build context** sent to the daemon before any stage runs - they are not a per-`COPY` filter, so no later stage can retrieve a path `.dockerignore` excluded, no matter how many `COPY --from=` hops happen afterward. The line was originally added because only `apps/website` consumed `docs/public` (and `apps/website` itself is `.dockerignore`d, since the Docker image doesn't build the website); once `apps/server`'s build gained its own reason to read the docs, the blanket exclusion silently broke it.
**Solution**: Removed the bare `docs` line from `.dockerignore`. The docs Markdown is ~60 small files (under 400K) with no secrets, so including them in the build context is cheap and safe - the runtime image still only ships the *compiled* `docs-index.sqlite` (verified with `find / -xdev -name '*.md'` inside a running container: nothing from the docs tree leaks in, only the pre-existing unrelated `apps/server/migrations/README.md`).
...
3. Place the merged binary in a firmwares dist directory as `<device_type>-client.bin`:
   ```bash
   mkdir -p ~/devicesdk/firmwares-dist
   cp firmware/esp32/build/esp32c3-client.bin ~/devicesdk/firmwares-dist/
   ```
4. Start the server with `FIRMWARES_DIST_DIR=~/devicesdk/firmwares-dist` so it seeds the artifact.
...
**Root Cause / non-obvious bits**: (1) The app is a plain Hono instance exported from `src/index.ts`; a test can boot the *real* server with `Bun.serve({ fetch: r => app.fetch(r, services), websocket })` on `port: 0` (ephemeral) using a fresh temp `DATA_DIR` + migrated SQLite per file. The `websocket` handler from `src/ws.ts` and the `services.server` handle must both be wired or upgrades never reach the route handlers. (2) `bun test` only counts files it actually loads, so importing `app` pulls in nearly the whole tree as the coverage denominator; the uploaded user-script *fixtures* (under the OS temp dir) and `@devicesdk/core/dist` get loaded too and must be excluded via `coveragePathIgnorePatterns` in `bunfig.toml` (use `**/tmp/**`, not `/tmp/**` - bun matches the *relative* display path, which begins `../../../tmp/...`). (3) **bun 1.3.14's `coverageThreshold` is unusable for a project-total gate**: the numeric form (`= 0.85`) enforces the minimum *per file* (BaseRoute-pattern endpoints can't meet it), and the `{ line = .., function = .. }` table form is **silently ignored** (exits 0 even at 0.999). (4) The rate limiter (`foundation/rateLimit.ts`) keys on client IP via a **process-global Map** shared across every `TestServer` in a run - register/login traffic trips 429 across files unless each request carries a unique `X-Forwarded-For`.
**Solution**: `apps/server/tests/harness.ts` provides `TestServer` (HTTP helpers + `register`/`login`/`scaffold`), `DeviceSim` (speaks the firmware WS protocol: `sendConnected`, `nextCommand`, `respond`/`ackNext`, `startAutoAck`), and `WatcherSim` (collects watch frames); it injects a random per-request `X-Forwarded-For`. The ≥85% **project-total** gate is `apps/server/scripts/coverage-gate.ts` (`pnpm --filter @devicesdk/server test:coverage`), which parses the "All files" row instead of using bun's threshold. CI runs it in a dedicated **Server E2E Tests** job; the plain **Test** job needs `oven-sh/setup-bun` because Bun isn't on the self-hosted runner's default PATH.
**Rule**: For bun coverage gates, don't rely on `coverageThreshold` if you want an aggregate (not per-file) bar - parse the total yourself. Any new `TestServer`-based test that hammers an auth route must vary the source IP. Bun e2e steps on the proxmox runners must `setup-bun` (it's not on PATH).

### Inter-device RPC `MAX_CALL_DEPTH` is unreachable via `this.env.DEVICES` (latent)
**Date**: 2026-06-13
**Question/Problem**: A mutual-recursion RPC test (`a.bounce()` → `b.bounce()` → `a.bounce()` …) never threw the `devicesBridge` "call depth limit (3) exceeded" error - it ran until the script's own counter guard returned.
**Root Cause**: `runtime/scriptHost.ts` `callMethod` builds a `scopedEnv` whose `DEVICES` proxy carries the incremented call depth, but invokes the user method via `scopedTarget[name]` whose proxy `get` returns `val.bind(t)` bound to the **raw target** - so inside the method body `this.env` resolves to the *original* frozen env (depth-0 `DEVICES` proxy), bypassing `scopedEnv`. Call depth never threads through nested `this.env.DEVICES[...]` calls, so `MAX_CALL_DEPTH` can't be hit that way. Not changed in the test PR (no behavior change) - flagged for follow-up.
**Rule**: When testing the RPC depth guard, drive `bridge()` with an explicit `callDepth`; don't expect `this.env.DEVICES` recursion to trip it. If fixing: bind the method to `scopedTarget` (or thread `scopedEnv` so `this.env` sees it).
...
**Remaining gap**: A device that stays half-open and *never* reconnects still won't resume until it reconnects; the firmware must detect dead sockets and reconnect. **Addressed for ESP32** (esp32 / esp32c3 / esp32c61): `firmware/esp32/main/devicesdk_main.c` now sets `ping_interval_sec` / `pingpong_timeout_sec` (protocol-level WebSocket PING/PONG) plus TCP keep-alive on the `esp_websocket_client_config_t`. The app-level `{"type":"ping"}` text frame was fire-and-forget (the server short-circuits it without replying - see the `webSocketMessage` ping early-exit at `device.ts`), so it could never detect a dead path. Cloudflare's runtime PONGs every protocol PING for free without waking the hibernating DO, so a missing PONG within `pingpong_timeout_sec` proves the path is dead → the client's built-in auto-reconnect fires → re-sends `device_connected` → server `rearmCronAlarmFromStorage()` resumes the cron. The steady ping traffic also keeps NAT mappings warm, avoiding the idle drop entirely. **Also addressed for Pico** (raw-lwIP client): `firmware/pico` now sends protocol-level PING frames every 60s (`WebsocketClient::send_ping()` from the main loop), PONGs inbound server pings, tracks the last received frame, and closes the connection in `poll()` when nothing arrives for 180s (3x the ping interval) - the 5s reconnect path then re-establishes the connection and re-sends `device_connected`.
...
2. Use `CONFIG_DEVICESDK_LED_IS_ADDRESSABLE` in Kconfig (defaults to `y` for `IDF_TARGET_ESP32C61`)
...
5. Intercept GPIO 8 and virtual pin 99 in `devicesdk_hal_set_gpio()` to route through led_strip driver
...
### ESP32 WebSocket handshake fails with "Header size exceeded buffer size"
**Date**: 2026-04-24
**Question/Problem**: ESP32 firmware connects to Wi-Fi, TLS cert validates (`esp-x509-crt-bundle: Certificate validated`), then the WebSocket Upgrade fails with `transport_ws: Header size exceeded buffer size` / `errno=119`. Device loops through `WEBSOCKET_EVENT_ERROR` every 10 s and never reaches `WEBSOCKET_EVENT_CONNECTED`. Symptoms on-device: LED never blinks 3× (server-connected), OLED stays blank because the server never gets to send `display_update`.
**Root Cause**: The ESP-IDF `tcp_transport` component uses a single buffer sized by `CONFIG_WS_BUFFER_SIZE` (default **1024 bytes**) for both the outgoing HTTP Upgrade request *and* the server's response headers. A fronted/proxied WebSocket upgrade response can carry many large headers (`cf-ray`, `set-cookie`, `alt-svc`, `nel`, `report-to`, `server`, `date`, `via`, `content-type`, etc.) - easily exceeding 1 KB. Check is at `transport_ws.c:308` inside a `do { esp_transport_read(... WS_BUFFER_SIZE - 1 - header_len ...) } while (header_len < WS_BUFFER_SIZE - 1)`. The `buffer_size` field of `esp_websocket_client_config_t` is **not** the same thing - that one sizes the WS frame rx/tx buffers, not the handshake buffer.
...
**Rule**: `CONFIG_WS_BUFFER_SIZE` must be at least 2048 whenever the firmware talks through a reverse proxy or CDN. Do not confuse this Kconfig with `esp_websocket_client_config_t.buffer_size` - they size unrelated buffers.
...
**Root Cause**: The task name `"websocket_task"` is the ESP-IDF `esp_websocket_client` component's *own* internal task (created at `managed_components/espressif__esp_websocket_client/esp_websocket_client.c:1285`), not our `websocket_task()` in `devicesdk_main.c`. Its default stack is `WEBSOCKET_TASK_STACK = 4*1024` bytes. The client dispatches event callbacks synchronously inside that task - so our `websocket_event_handler` (cJSON parse + `ESP_LOGI("Received: %s", …)` + `handle_websocket_message` → `queue_command`) runs there too. Measured peak usage on C3: **~8.9 KB**. 4 KB overflows hard; even 8 KB falls short by ~700 B.
