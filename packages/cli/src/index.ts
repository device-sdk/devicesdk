...
program
    .name('devicesdk')
    .description('CLI tool for developing and deploying DeviceSDK IoT applications')
    .version('0.1.0');
...
program
    ...
    .option('--template <name>', 'Use a starter template (basic, multi-device, empty)', 'basic')
...
program
    ...
    .option('-c, --config <path>', 'Path to the devicesdk.ts config file')
    .option('-t, --timeout <ms>', 'Time to wait for device (ms)', (value) => parseInt(value, 10))
    .option('-p, --port <port>', 'Serial port for ESP32 devices (e.g., /dev/cu.usbserial-0001)')
    .option('-b, --baud <rate>', 'Baud rate for ESP32 flashing (default: 460800)', (value) => parseInt(value, 10))
    .option('--host <url>', 'Download firmware from a custom host (e.g., http://192.168.0.1:9000)')
