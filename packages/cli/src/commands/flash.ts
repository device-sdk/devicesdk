...
import { flashESP32, checkEsptoolInstalled } from '../flash/esp32.js';
...
async function ensureDeviceExists(token: string, projectId: string, deviceId: string, name?: string, description?: string): Promise<void> {
  try {
...
export default async function flash(deviceId: string, options: FlashOptions = {}): Promise<void> {
  if (!deviceId) {
  ...
  try {
    ...
    await ensureDeviceExists(token, config.projectId, deviceId, deviceConfig.name, deviceConfig.description);
    ...
    if (isEsp32DeviceType(deviceType)) {
      ...
      if (!hasEsptool) {
        ...
        console.error('  Or see: https://docs.espressif.com/projects/esptool/en/latest/');
        process.exit(6);
      }
      ...
      const firmwareDir = path.join(configDir, '.devicesdk', 'firmware', deviceId);
      ...
      const result = await flashPico({ firmwarePath, timeoutMs: options.timeout ?? 120_000 });
