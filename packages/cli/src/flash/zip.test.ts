...
describe('extractESP32Firmware', () => {
  it('extracts valid firmware archive', () => {
    ...
    expect(firmware.files['bootloader.bin']).toEqual(Buffer.from([0x00, 0x01, 0x02]));
    expect(firmware.files['partition-table.bin']).toEqual(Buffer.from([0x03, 0x04, 0x05]));
  ...
  it('throws when flasher_args.json is missing', () => {
    ...
    expect(() => extractESP32Firmware(zipBuffer)).toThrow('missing flasher_args.json');
  });
