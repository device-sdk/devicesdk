...
- [**Raspberry Pi Pico W**](/hardware/pico-w/) - RP2040 dual Cortex-M0+, 2.4GHz WiFi. Full support.
- [**Raspberry Pi Pico 2W**](/hardware/pico-2w/) - RP2350 dual Cortex-M33, 2.4GHz WiFi. Full support, Pico W pin-compatible.
- [**ESP32**](/hardware/esp32/) - classic dual Xtensa LX6, 2.4GHz WiFi. Full support.
- [**ESP32-C3**](/hardware/esp32-c3/) - single-core RISC-V, 2.4GHz WiFi. Full support.
- [**ESP32-C61**](/hardware/esp32-c61/) - single-core RISC-V, 2.4GHz WiFi 6. Full support.
...
See the [flash command reference](/cli/flash/) for end-to-end flashing instructions.
...
**ESP32 family:** Install esptool (`pip install esptool`); on Linux add your user to the `dialout` group (`sudo usermod -a -G dialout $USER`, then log out/in). If you see "No serial data received", your board may not auto-reset - use manual boot mode and `--before no_reset` (see [flash docs](/cli/flash/)). Try the USB-JTAG port (`/dev/ttyACM0`) instead of the UART port (`/dev/ttyUSB0`), or lower the baud rate with `--baud 115200`.
...
- [Your First Device](/first-device/) - build an entrypoint.
- [Quickstart](/quickstart/) - flash and deploy in under 15 minutes.
- [CLI Flash Command](/cli/flash/) - detailed flashing reference.
