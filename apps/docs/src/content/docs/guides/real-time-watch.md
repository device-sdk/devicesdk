...
User code can also emit custom state events from device scripts with `this.env.DEVICE.emitState(entity_id, value)` - see [Emit State](/concepts/emit-state/) for details.
