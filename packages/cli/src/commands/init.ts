...
function generateConfigFile(projectId: string, devices: Record<string, { entrypoint: string; name?: string; description?: string }>): string {
  const devicesStr = Object.entries(devices)
    .map(([key, value]) => {
      const props = [`      entrypoint: "${value.entrypoint}"`];
      if (value.name) props.push(`      name: "${value.name}"`);
      if (value.description) props.push(`      description: "${value.description}"`);
      return `    "${key}": {\n${props.join(',\n')},\n    }`;
    })
    .join(',\n');
...
export default async function init(projectIdArg?: string, options: InitOptions = {}): Promise<void> {
  ...
  const projectDir = projectIdArg ? path.resolve(process.cwd(), projectId) : process.cwd();
  ...
  try {
    ...
    await fs.writeFile(configPath, generateConfigFile(projectId, template.devices));
