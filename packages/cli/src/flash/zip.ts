...
import { ESP32Firmware, ESP32FlasherArgs } from '../api.js';
...
export function extractESP32Firmware(zipBuffer: Buffer): ESP32Firmware {
  ...
  if (!flasherArgs) {
    throw new Error('Invalid ESP32 firmware archive: missing flasher_args.json');
  }

  // Validate all expected files are present
  for (const [address, filename] of Object.entries(flasherArgs.flash_files)) {
    if (!files[filename]) {
      throw new Error(`Invalid ESP32 firmware archive: missing ${filename} (expected at ${address})`);
    }
  }
