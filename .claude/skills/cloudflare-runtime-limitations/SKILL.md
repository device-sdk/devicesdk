...
- **Never cache a `getTarget()` handle (or any dynamic-worker RPC stub) across invocations.** The handle is scoped to the invocation that resolved it; once that invocation ends the child isolate is recycled and the stub is stale. Calling a method on a stale cross-invocation stub does **not** throw a clean error — it **fans out subrequests that never resolve**, blows the per-invocation subrequest cap, and pins the DO at the ~60 s wall (`wallTime≈60000`, `cpuTime 0`, and your own timing deltas read 0 ms because no I/O completes). For a per-device cron DO this froze the single-threaded actor every minute and starved the device WebSocket PING/PONG into a reconnect loop. Re-resolve the worker **per invocation** (a stable `id` keeps it one underlying worker, so you don't hit "Too many concurrent dynamic workers"); cache the resolved stub only *within* a single invocation. In `device.ts` the cache (`cachedUserWorker`) is cleared at every entry point that resolves a worker (`alarm()`, `handleRemoteCall()`) — see PR #140 and the 2026-06-04 TROUBLESHOOT entry.
...
| DO alarm pinned at `wallTime≈60000`, `cpuTime 0`, `Too many subrequests` | Cached `getTarget()` stub reused across invocations (stale) | Re-resolve the worker per invocation; clear the cache at entry points |
