...
vi.mock('../api.js', async (importOriginal) => {
  ...
  return {
    ...
    downloadDeviceFirmware: (...args: any[]) => apiMocks.downloadDeviceFirmware(...args),
    downloadESP32Firmware: (...args: any[]) => apiMocks.downloadESP32Firmware(...args),
  };
});
...
vi.mock('../utils.js', () => ({
  loadConfig: vi.fn().mockImplementation(() => Promise.resolve(createBaseConfig())),
...
describe('flash command', () => {
  ...
  it('flashes successfully when project and device exist', async () => {
    ...
    expect(apiMocks.getProject).toHaveBeenCalledWith('test-token', 'test-project');
    expect(apiMocks.getDevice).toHaveBeenCalledWith('test-token', 'test-project', 'pico-1');
    expect(apiMocks.downloadDeviceFirmware).toHaveBeenCalledWith(
      'test-token',
      'test-project',
      'pico-1',
    ...
    expect(flashPicoMock).toHaveBeenCalledWith(expect.objectContaining({ firmwarePath: expect.stringContaining('pico-1.uf2') }));
    expect(exitSpy).not.toHaveBeenCalled();
  });

  it('creates project when missing', async () => {
    apiMocks.getProject.mockRejectedValueOnce(new DeviceSDKApiError('missing', 404));

    await flash('pico-1');

    expect(apiMocks.createProject).toHaveBeenCalledWith('test-token', 'test-project');
  });

  it('creates device when missing', async () => {
    apiMocks.getDevice.mockRejectedValueOnce(new DeviceSDKApiError('missing', 404));
  ...
  it('exits when device is not in config', async () => {
    ...
    (loadConfig as any).mockResolvedValueOnce({ ...createBaseConfig(), devices: {} });
  ...
  it('exits when firmware download fails', async () => {
    apiMocks.downloadDeviceFirmware.mockRejectedValueOnce(new Error('download failed'));
  ...
  it('exits on non-404 project API error', async () => {
    apiMocks.getProject.mockRejectedValueOnce(new DeviceSDKApiError('boom', 500));
  ...
  describe('ESP32 devices', () => {
    ...
    it('exits when ESP32 firmware download fails', async () => {
      const { loadConfig } = await import('../utils.js');
      (loadConfig as any).mockResolvedValueOnce(createEsp32Config());
      apiMocks.downloadESP32Firmware.mockRejectedValueOnce(new Error('download failed'));
    ...
    it('exits when ESP32 flashing fails', async () => {
      const { loadConfig } = await import('../utils.js');
      (loadConfig as any).mockResolvedValueOnce(createEsp32Config());
      flashEsp32Mocks.flashESP32.mockRejectedValueOnce(new Error('flash failed'));
