...
- **Content** - `content/` only. The documentation lives in the separate
  `apps/docs` Nimbus app and is served from `https://docs.devicesdk.com`.
...
managed cloud. Public-facing copy under `content/` must describe the self-hosted
reality:
...
Whenever you rename, move, or delete a content file under `content/`, you
**MUST** add a 301 entry to `apps/website/static/_redirects` mapping the old URL
to the new one (or to the closest still-existing parent for deletions).
...
`/old-section/:slug /new-section/:slug 301`. Place the more specific entries
before the general ones.

The docs site (`https://docs.devicesdk.com`) is a separate subdomain. The
marketing site already redirects all `/docs/*` URLs there preserving the path.
Do **not** add marketing-site redirects for docs-only URL changes - those
redirects belong in `apps/docs` if Nimbus supports them.
...
- The file is under `apps/docs/src/content/docs/` (the docs subdomain, not the
  marketing site).
