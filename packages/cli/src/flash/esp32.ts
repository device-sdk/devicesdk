  ...
  if (platform === 'darwin') {
    try {
      ...
      for (const entry of devDir) {
        if (entry.startsWith('cu.usb') || entry.startsWith('cu.SLAB_USBtoUART') || entry.startsWith('cu.wchusbserial')) {
...
export async function detectESP32Port(timeoutMs: number = DEFAULT_TIMEOUT): Promise<string> {
...
export async function flashESP32(options: ESP32FlashOptions): Promise<{ port: string }> {
  ...
  const args: string[] = [
    '--chip', firmware.flasherArgs.chip,
    '--port', port,
    '--baud', String(baud),
    '--before', firmware.flasherArgs.before,
    '--after', firmware.flasherArgs.after,
    'write_flash',
    '--flash_mode', firmware.flasherArgs.flash_mode,
    '--flash_size', firmware.flasherArgs.flash_size,
    '--flash_freq', firmware.flasherArgs.flash_freq,
  ];

  // Add flash file addresses and paths
  for (const [address, filename] of Object.entries(firmware.flasherArgs.flash_files)) {
