 ...
 * Data stored as 5 bytes per char (one per column, LSB = top row)
...
// Font data: 5 bytes per character, columns from left to right
// Each byte is a column with bit 0 = top pixel, bit 6 = bottom pixel
...
export function getCharData(char: string, font: Font = font5x7): Uint8Array | undefined {
	...
	const offset = (code - font.firstChar) * font.width;
	return font.data.slice(offset, offset + font.width);
}
