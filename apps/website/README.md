...
Marketing website for [DeviceSDK](https://devicesdk.com) - the free, open-source,
**self-hosted** IoT platform. This is the public site at `devicesdk.com`; it is
**not** part of the product (there is no hosted dashboard or managed cloud - users run
the DeviceSDK server on their own hardware).

The documentation now lives at [docs.devicesdk.com](https://docs.devicesdk.com) (the
`apps/docs` Nimbus/Astro app). Old `/docs/*` URLs on this domain redirect there with a
301 preserving the path.
...
The full build depends on `apps/server/openapi.json` (copied into the `/api/` reference),
...
If you rename, move, or delete a page under `content/`, add a matching 301 to
`static/_redirects` so the old URL keeps its search-index signal. The docs are now on a
separate subdomain (`https://docs.devicesdk.com`); the marketing site's `_redirects`
handles `/docs/*` redirects. See `apps/website/AGENTS.md` for the full rule and format.
