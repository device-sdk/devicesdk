...
- ✅ UART serial - 2 ports
- ✅ 1-Wire (DS18B20) - multi-probe discovery by ROM code (via the **UART1 peripheral**, see below)
...
- **1-Wire uses UART1.** The C61 has no RMT peripheral, so the 1-Wire bus is driven through the UART1 peripheral. Don't use the `uart*` commands on port 1 while a DS18B20 is wired up.
...
- **Bootloader offset `0x0`.** Same layout as ESP32-C3. Pre-built artifact is `esp32c61-client.bin`; see the [local flashing section in AGENTS.md](/cli/flash/#esp32-flashing-process) for the exact flash layout.
...
See the [flash command reference](/cli/flash/#esp32-flashing-process) for the canonical walkthrough.
